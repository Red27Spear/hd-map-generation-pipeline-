...
def ecef2lla(x, y, z, *left):
    ...
    if ( type(x) == type(np.array([1])) ): # array of coordinates
        idx_ = np.where((np.fabs(x) < 1) & (np.fabs(y) < 1))[0]
        alt[idx_] = np.fabs(z[idx_]) -b
...
#          Zone- vector of UTM zones
    ...
    numPts = lat.size
    step_size = int(1e07)
    N_all = np.zeros((numPts,))
    E_all = np.zeros((numPts,))

    for i in range(0,numPts,step_size):
        idx = np.arange(i,np.min((i+step_size,numPts)))
        if idx.size == 1:
            lon_curr = np.array([lon])
            lat_curr = np.array([lat])
        else:
            lon_curr = lon[idx]
            lat_curr = lat[idx]
        ...
        No=np.zeros( lat_curr.shape )  # False northing (north)
        # No(lat<0)=1e7           # False northing (south)
        np.where(lat_curr < 0, No, 1e7)
        ...
        f = 1 - np.sqrt(1 - e2)
        RN = a / ((1 - e2 * np.sin(lat_curr)**2))**0.5
        RM = a*(1-e2)/(1-e2 * np.sin(lat_curr)**2)**1.5
        t = np.tan(lat_curr)
        h = np.sqrt((e2*np.cos(lat_curr)**2) / (1 - e2))
        n = f/(2-f)
        ...
        s=a/(1+n)*(a0*lat_curr-a2*np.sin(2*lat_curr)+a4*np.sin(4*lat_curr)-a6*np.sin(6*lat_curr)+a8*np.sin(8*lat_curr))

        E1=lam * np.cos(lat_curr)
        E2=lam**3 * np.cos(lat_curr)**3/6 * (1-t**2+h**2)
        E3=lam**5 * np.cos(lat_curr)**5/120 * (5-18*t**2+t**4+14*h**2-58*t**2*h**2+13*h**4+4*h**6-64*t**2*h**4-24*t**2*h**6)
        E4=lam**7 * np.cos(lat_curr)**7/5040*(61-479*t**2+179*t**4-t**6)

        E=Eo + ko*RN*(E1 + E2 + E3 + E4)

        N1=lam**2/2 * np.sin(lat_curr) * np.cos(lat_curr)
        N2=lam**4/24 * np.sin(lat_curr) * np.cos(lat_curr)**3*(5-t**2+9*h**2+4*h**4)
        N3=lam**6/720 * np.sin(lat_curr) * np.cos(lat_curr)**5*(61-58*t**2+t**4+270*h**2-330*t**2*h**2+445*h**4+324*h**6-680*t**2*h**4+88*h**8-600*t**2*h**6-192*t**2*h**8)
        N4=lam**8/40320*np.sin(lat_curr)*np.cos(lat_curr)**7*(1385-311*t**2+543*t**4-t**6)
